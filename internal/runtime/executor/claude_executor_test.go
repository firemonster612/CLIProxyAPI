...
func modelFreeClaudeCAISSignatureForExecutorTest(envelopeVersion, channelID uint64) string {
	...
	// CAQS (envelope version >= 4) requires a thinking/narration block kind;
	// real captures carry field 8 on every generation.
	channel = protowire.AppendTag(channel, 8, protowire.BytesType)
	channel = protowire.AppendBytes(channel, []byte("thinking"))
