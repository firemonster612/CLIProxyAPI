...
func assertClaudeNativeHelperHeaders(t *testing.T, got, incoming http.Header) {
	...
	// 2.1.258 attaches x-client-request-id only for a first-party base URL, so
	// a helper that arrived without one keeps it absent on a custom gateway
	// rather than gaining a synthesized ID the real client would not have sent.
	if requestID := claudeNativeHelperHeaderValue(got, "X-Client-Request-Id"); requestID != "" {
		t.Fatalf("X-Client-Request-Id = %q, want absent", requestID)
	}
}
