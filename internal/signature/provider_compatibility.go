...
func detectSignatureProviderForBlock(rawSignature string, blockKind SignatureBlockKind) signatureProviderDetection {
	...
	//      model text or the model-free channel/container structure (from
	//      envelope version 4, upstream's CAQS layout with the signature bytes
	//      relocated to container field 5).
...
func claudeCompatibleSignatureReason(targetProvider SignatureProvider, rawSignature, targetModel string) string {
	...
	case info.EnvelopeVersion >= 4:
		reason = "valid Claude model-free CAQS thinking signature is compatible with any Claude target"
	default:
